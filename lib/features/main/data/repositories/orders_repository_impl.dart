...
class OrdersRepositoryImpl extends OrdersRepository {
  ...
  @override
  Future<Either<Failure, BaseModel<SelectedOrderEntity>>> getSelectedOrder(int orderId) async {
    try {
      final result = await dataSource.getSelectedOrder(orderId);
      return Right(result);
    } on ServerException catch (e) {
      return Left(ServerFailure(errorMessage: e.errorMessage, statusCode: e.statusCode));
    } on ParsingException catch (e) {
  ...
  @override
  Future<Either<Failure, void>> addNewService({required String title, required double price}) async {
    try {
      final result = await dataSource.addNewService(title: title, price: price);
      return Right(result);
    } on ServerException catch (e) {
      return Left(ServerFailure(errorMessage: e.errorMessage, statusCode: e.statusCode));
    } on ParsingException catch (e) {
