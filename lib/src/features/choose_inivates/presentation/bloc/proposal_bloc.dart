...
import 'package:taxi_app/src/features/choose_inivates/presentation/bloc/proposal_event.dart';
...
class ProposalBloc extends Bloc<ProposalEvent, ProposalState> {
  ...
  ProposalBloc(this.homeRepository)
      : super(const ProposalState(status: ProposalStatus.initial)) {
        ...
        try {
          ...
          if (proposalData != null) {
            ...
            emit(const ProposalState(
              ...
              errorMessage: 'No proposal data received',
            ));
          }
        } catch (e) {
          emit(ProposalState(
            status: ProposalStatus.error,
            errorMessage: 'Failed to parse proposal: $e',
          ));
        }
      } else {
        emit(ProposalState(
