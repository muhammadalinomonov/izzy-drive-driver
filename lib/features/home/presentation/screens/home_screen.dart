...
import 'package:taxi_app/core/widgets/coming_soon_toast.dart';
...
import 'package:taxi_app/features/home/presentation/widgets/active_order_widget.dart';
import 'package:taxi_app/features/home/presentation/widgets/search_input.dart';
...
import 'package:taxi_app/features/phone_verify/presentation/widgets/phone_verify_sheet.dart';
      ...
      builder: (context, state) {
        if (state.status == HomeStatus.loading) {
          return const Padding(
            ...
            child: AppSkeleton(
              child: SkeletonBox(height: 137, radius: 16),
            ),
          );
  ...
  Widget build(BuildContext context) {
    return const AppSkeleton(
      child: Padding(
        ...
        child: Column(
          children: [
            _SearchAndRecentsBlock(),
            SizedBox(height: 16),
            // "How it works" / "Other opportunities" bloki.
            SkeletonBox(height: 232, radius: 16),
            SizedBox(height: 20),
            SkeletonBox(height: 137, radius: 16),
          ],
        ),
      ),
    );
...
class _HomeTopSkeleton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const AppSkeleton(child: _SearchAndRecentsBlock());
  }
}

/// Search field over two recent-address rows — the shape both the whole-page
/// and the top-only skeleton share.
class _SearchAndRecentsBlock extends StatelessWidget {
  const _SearchAndRecentsBlock();

  @override
  Widget build(BuildContext context) {
    return const Column(
      children: [
        SkeletonBox(height: 50, radius: 50),
        SizedBox(height: 12),
