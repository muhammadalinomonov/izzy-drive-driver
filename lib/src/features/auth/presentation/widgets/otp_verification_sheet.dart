...
Future<bool?> showOtpVerificationSheet(BuildContext context) {
  ...
  return showModalBottomSheet<bool>(
    ...
    builder: (sheetContext) {
      return BlocProvider.value(
        value: bloc,
        child: const _OtpVerificationSheet(),
      );
    },
  );
}
...
class _OtpVerificationSheetState extends State<_OtpVerificationSheet> {
  ...
  void initState() {
    super.initState();
    _startTimer(context.read<AuthBloc>().state.resendAfter);
  }
      ...
      listener: (context, state) {
        ...
        if (state.verifyOtpStatus == AuthStatus.success) {
          ...
          AppSnackBar.showError(
            context,
            state.errorMessage ?? LocaleKeys.auth_otp_verifyFailed.tr(),
          );
                ...
                TextButton(
                  onPressed: isVerifying
                      ...
                      : () {
                          context
                              .read<AuthBloc>()
                              .add(const ResetRegistrationEvent());
